...
export class CameraProperty {
     ...
     * @enum {number}
     */
    static readonly ID: Record<'EFCompensation' | 'ProductName' | 'ICCProfile' | 'OwnerName' | 'GPSDateStamp' | 'MakerName' | 'ColorSpace' | 'DateTime' | 'FirmwareVersion' | 'RedEye' | 'BatteryLevel' | 'Orientation' | 'CFn' | 'MirrorUpSetting' | 'SaveTo' | 'CurrentStorage' | 'WhiteBalance' | 'CurrentFolder' | 'PictureStyleDescription' | 'MyMenu' | 'TimeZone' | 'BatteryQuality' | 'UTCTime' | 'BodyIDEx' | 'HDDirectoryStructure' | 'GPSSatellites' | 'TemperatureStatus' | 'LensStatus' | 'JpegQuality' | 'ImageQuality' | 'FocusInfo' | 'FlashMode' | 'ColorTemperature' | 'WhiteBalanceShift' | 'ExposureCompensation' | 'PictureStyle' | 'PictureStyleCaption' | 'AFMode' | 'PowerZoom_Speed' | 'GPSVersionID' | 'Av' | 'GPSLatitudeRef' | 'GPSLatitude' | 'AEModeSelect' | 'FEBracket' | 'GPSLongitudeRef' | 'AEMode' | 'GPSLongitude' | 'DriveMode' | 'GPSAltitudeRef' | 'GPSAltitude' | 'FlashOn' | 'GPSTimeStamp' | 'GPSStatus' | 'GPSMapDatum' | 'Tv' | 'ISOSpeed' | 'MeteringMode' | 'FlashCompensation' | 'FocalLength' | 'AvailableShots' | 'Bracket' | 'WhiteBalanceBracket' | 'LensName' | 'AEBracket' | 'ISOBracket' | 'NoiseReduction' | 'Artist' | 'Copyright' | 'DepthOfField' | 'Evf_OutputDevice' | 'Evf_Mode' | 'Evf_WhiteBalance' | 'Evf_ColorTemperature' | 'Evf_DepthOfFieldPreview' | 'Evf_Zoom' | 'Evf_ZoomPosition' | 'Evf_FocusAid' | 'Evf_Histogram' | 'Evf_ImagePosition' | 'Evf_HistogramStatus' | 'Evf_AFMode' | 'Record' | 'Evf_HistogramY' | 'Evf_HistogramR' | 'Evf_HistogramG' | 'Evf_HistogramB' | 'Evf_CoordinateSystem' | 'Evf_ZoomRect' | 'Evf_ImageClipRect' | 'Evf_PowerZoom_CurPosition' | 'Evf_PowerZoom_MaxPosition' | 'Evf_PowerZoom_MinPosition' | 'EVF_RollingPitching' | 'FixedMovie' | 'MovieParam' | 'Evf_ClickWBCoeffs' | 'ManualWhiteBalanceData' | 'MirrorLockUpState' | 'SummerTimeSetting' | 'AutoPowerOffSetting' | 'DC_Zoom' | 'DC_Strobe' | 'LensBarrelStatus', number> = {
        'AEBracket': 1038,
        'AEMode': 1024,
        'AEModeSelect': 1078,
        'AFMode': 1028,
